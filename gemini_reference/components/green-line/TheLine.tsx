...
export default function TheLine() {
  ...
  return (
    ...
    >
      ...
      >
        {/* DELETED the <defs> and <filter> block entirely */}
        <path
          ref={pathRef}
          d={createPath(containerHeight)}
          stroke="#C6F432"
          strokeWidth={isMobile ? "1" : "1.5"} // Keeps your exact original size
          fill="none"
          strokeLinecap="round"
          strokeLinejoin="round"
          // DELETED filter="url(#glow-line)" attribute
          style={{
            // Stacked CSS drop-shadows: 
            // 1. A tight 4px shadow for the bright core
            // 2. A wider 12px shadow for the ambient neon bloom
            filter: isMobile 
              ? 'drop-shadow(0px 0px 4px rgba(198, 244, 50, 0.6))'
              : 'drop-shadow(0px 0px 4px rgba(198, 244, 50, 0.9)) drop-shadow(0px 0px 12px rgba(198, 244, 50, 0.4))',
